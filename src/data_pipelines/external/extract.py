...
class APIEndpoints:
    ...
    dwd_mosmix_stations: str = "DE/leistungen/met_verfahren_mosmix/mosmix_stationskatalog.cfg?view=nasPublication"
    dwd_mosmix_single_station: str = "weather/local_forecasts/mos/MOSMIX_L/single_stations/{station_id}/kml/MOSMIX_L_LATEST_{station_id}.kmz"
    
    # DWD precipitation endpoints
    dwd_precipitation_stations: str = "climate_environment/CDC/observations_germany/climate/10_minutes/precipitation/now/zehn_now_rr_Beschreibung_Stationen.txt"
    dwd_precipitation_data: str = "climate_environment/CDC/observations_germany/climate/10_minutes/precipitation/now/10minutenwerte_nieder_{station_id}_now.zip"
...
class DataExtractor:
    ...
    def __init__(self, timeout: float = 60.0):
        ...
        self._clients = {
            'dwd_opendata': httpx.Client(base_url=self.base_urls.dwd_opendata, timeout=timeout),
            'dwd': httpx.Client(base_url=self.base_urls.dwd, timeout=timeout),
            'pegelonline': httpx.Client(base_url=self.base_urls.pegelonline, timeout=timeout),
        }
    ...
    def fetch_dwd_mosmix_single_station(
        ...
        endpoint = self.endpoints.dwd_mosmix_single_station.format(station_id=station_id)
        response = client.get(endpoint)
        response.raise_for_status()
    ...
    def fetch_dwd_precipitation_stations(self) -> list[DWDTenMinNowPercipitationStations]:
